...
class _ThumbnailGridState extends State<ThumbnailGrid> {
  ...
  Future<void> _renderPage(int pageIndex) async {
    ...
    if (_cache.containsKey(pageIndex)) return;
    // Atomic check-and-add: Set.add returns false if the entry was already
    // present. Using it guards against a race where two concurrent callers
    // (e.g. two scroll events landing in the same frame) both pass the
    // containsKey/contains check and then both kick off rendering, wasting
    // work and potentially leaking PdfImage handles.
    if (!_pending.add(pageIndex)) return;

    // Capture the current document so we can detect a switch mid-render.
    // Without this, in-flight renders from the previous document would
    // land in `_cache` after didUpdateWidget cleared it, producing
    // thumbnails for the wrong document.
    ...
    try {
      ...
      // Re-check the document identity before caching the result.
      if (mounted && _document == capturedDoc) {
        setState(() {
        ...
        // Either the widget was disposed or the document was swapped
        // while we were rendering — don't leak the new ui.Image.
        uiImage.dispose();
        _pending.remove(pageIndex);
      }
    } catch (_) {
