...
Future<bool> showPassphraseDialog(BuildContext context) async {
  ...
  final passphrase = result;
  if (passphrase == null || passphrase.isEmpty) return false;
  if (!context.mounted) return false;

  // Capture provider before any further work — provider is app-scoped so this
  // is safe, but we still early-out if the calling context has gone away so
  // the caller doesn't think the passphrase was applied when it wasn't.
  final encryption = context.read<EncryptionProvider>();
  encryption.setPassphrase(passphrase);
  // Persist so the passphrase survives an app restart and can be restored on
  // startup (or used by the biometric unlock flow). Keystore-encrypted —
  // strictly stronger than plain SharedPreferences.
  await BiometricPassphraseStorage().write(passphrase);
  return true;
}

/// The dialog body, a StatefulWidget that owns its [TextEditingController] so
/// the controller is never disposed while the closing route animation is still
/// referencing it (which previously triggered framework assertions).
...
class _PassphraseDialogState extends State<_PassphraseDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final isCommon = text.length >= 8 && isCommonPassword(text);
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        FilledButton(
          onPressed: isValid ? () => Navigator.pop(context, _controller.text) : null,
